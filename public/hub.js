...
const loadSlates = async () => {
  ...
  document.querySelectorAll("[data-delete-slate]").forEach((button) => button.addEventListener("click", async () => {
    if (!window.confirm("Apagar esta chapa definitivamente? Esta ação não pode ser desfeita.")) return;
    try { await readJson(await fetch("/api/admin/slates", { method: "DELETE", headers: authHeaders(true), body: JSON.stringify({ id: button.dataset.deleteSlate }) })); await loadSlates(); }
    catch (error) { const target = document.querySelector(`[data-slate-message="${button.dataset.deleteSlate}"]`); target.textContent = error.message; target.className = "form-message error"; }
  }));
};
