...
const renderCalendar = (dates) => Object.entries(dates).map(([key, value]) => `<article class="timeline-item ${value ? "is-defined" : ""}"><strong>${escapeHtml(dateKey(key))}</strong><time>${escapeHtml(formatDate(value))}</time></article>`).join("");
