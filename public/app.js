...
const renderSlates = (slates) => {
  ...
  return slates.map((slate) => { const president = (slate.integrantes || []).find((member) => /presid|coordenação geral/i.test(member.cargo)); const vice = (slate.integrantes || []).find((member) => /vice/i.test(member.cargo)); const proposals = (slate.propostas || []).slice(0, 3); return `<article class="slate-card"><div class="slate-card-head"><span class="slate-number">CHAPA <strong>${escapeHtml(String(slate.numero))}</strong></span><span class="slate-status"><i></i> Candidatura homologada</span></div><h3>${escapeHtml(slate.nome)}</h3><p class="slate-summary">${escapeHtml(slate.apresentacao || "Informações públicas da chapa ainda não foram adicionadas.")}</p><div class="slate-leaders"><small>REPRESENTAÇÃO PRINCIPAL</small><div><span><b class="leader-icon material-symbols-outlined">person</b><em>Presidência</em><strong>${escapeHtml(president?.nome || "A informar")}</strong></span><span><b class="leader-icon material-symbols-outlined">person</b><em>Vice-presidência</em><strong>${escapeHtml(vice?.nome || "A informar")}</strong></span></div></div>${proposals.length ? `<div class="slate-priority"><small>EIXOS PRIORITÁRIOS DE GESTÃO</small>${proposals.map((proposal) => `<p><span class="material-symbols-outlined">check_circle</span><strong>${escapeHtml(proposal.titulo)}:</strong> ${escapeHtml(proposal.descricao)}</p>`).join("")}</div>` : ""}<a class="slate-more" href="/chapa?id=${encodeURIComponent(slate.id)}">Ver informações completas <span aria-hidden="true">→</span></a><div class="slate-card-foot">Processo eleitoral estudantil <span>Urna: ${escapeHtml(String(slate.numero))}</span></div></article>`; }).join("");
};
