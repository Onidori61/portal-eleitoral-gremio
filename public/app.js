...
const renderSlates = (slates) => {
  ...
  return slates.map((slate) => { const president = (slate.integrantes || []).find((member) => /presid|coordenação geral/i.test(member.cargo)); const vice = (slate.integrantes || []).find((member) => /vice/i.test(member.cargo)); const proposals = (slate.propostas || []).slice(0, 3); return `<article class="slate-card"><div class="slate-card-head"><span class="slate-number">Chapa ${escapeHtml(String(slate.numero))}</span></div><h3>${escapeHtml(slate.nome)}</h3><p class="slate-summary">${escapeHtml(slate.apresentacao || "Informações públicas da chapa ainda não foram adicionadas.")}</p><div class="slate-leaders"><small>Representação principal</small><div><span><em>Presidência</em><strong>${escapeHtml(president?.nome || "A informar")}</strong></span><span><em>Vice-presidência</em><strong>${escapeHtml(vice?.nome || "A informar")}</strong></span></div></div>${proposals.length ? `<div class="slate-priority"><small>Propostas em destaque</small>${proposals.map((proposal) => `<p><strong>${escapeHtml(proposal.titulo)}:</strong> ${escapeHtml(proposal.descricao)}</p>`).join("")}</div>` : ""}<a class="slate-more" href="/chapa?id=${encodeURIComponent(slate.id)}">Ver informações completas <span aria-hidden="true">→</span></a><div class="slate-card-foot">Processo eleitoral estudantil <span>Urna: ${escapeHtml(String(slate.numero))}</span></div></article>`; }).join("");
};
...
const calendarOrder = ["inscricoesInicio", "inscricoesFim", "campanhaInicio", "campanhaFim", "votacaoInicio", "votacaoFim", "apuracao"];
const renderCalendar = (dates) => calendarOrder.map((key) => [key, dates?.[key]]).map(([key, value]) => `<article class="timeline-item ${value ? "is-defined" : ""}"><strong>${escapeHtml(dateKey(key))}</strong><time>${escapeHtml(formatDate(value))}</time></article>`).join("");
